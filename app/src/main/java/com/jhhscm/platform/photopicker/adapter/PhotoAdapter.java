...
public class PhotoAdapter extends BaseAdapter {
    ...
    //图片选择数量
    private int mMaxNum = PhotoPickerActivity.DEFAULT_MAX_TOTAL;

    private View.OnClickListener mOnPhotoClick;
    private PhotoClickCallBack mCallBack;

    public PhotoAdapter(Context context, List<Photo> mDatas, int itemWidth) {
        this.mDatas = mDatas;
     ...
     * 通过图片路径设置默认选择
     */
    public void setDefaultSelected(ArrayList<String> selectPhotos) {
        mSelectedPhotos = selectPhotos;
    }
    ...
    public void removeImage(String delImage) {
        if (mSelectedPhotos != null && mSelectedPhotos.contains(delImage)) {
            mSelectedPhotos.remove(delImage);
        }
    ...
    private void initMultiMode() {
        ...
        mOnPhotoClick = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String path = FileUtils.getPhotoImageUri(v.findViewById(R.id.iv_album).getTag().toString());
                if (mSelectedPhotos.contains(path)) {
                    v.findViewById(R.id.v_mask).setVisibility(View.GONE);
                    v.findViewById(R.id.chk_btn).setSelected(false);
                    mSelectedPhotos.remove(path);
                } else {
                    if (mSelectedPhotos.size() >= mMaxNum) {
                        Toast.makeText(mContext, R.string.photopicker_msg_amount_limit,
                                Toast.LENGTH_SHORT).show();
                        return;
                    }
                    mSelectedPhotos.add(path);
                    v.findViewById(R.id.v_mask).setVisibility(View.VISIBLE);
                    v.findViewById(R.id.chk_btn).setSelected(true);
                }
                if (mCallBack != null) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (getItemViewType(position) == TYPE_CAMERA) {
            convertView = LayoutInflater.from(mContext).inflate(
                    R.layout.item_photopicker_camera, null);
            ...
            if (convertView == null) {
                ...
                convertView = LayoutInflater.from(mContext).inflate(
                        R.layout.item_photopicker_photo, null);
                GridView.LayoutParams lp = new GridView.LayoutParams(mWidth, mWidth);
                convertView.setLayoutParams(lp);
                holder.photoImageView = (ImageView) convertView.findViewById(R.id.iv_album);
                holder.selectView = (ImageView) convertView.findViewById(R.id.chk_btn);
                holder.maskView = convertView.findViewById(R.id.v_mask);
                holder.wrapLayout = (FrameLayout) convertView.findViewById(R.id.fl_wrap);
                convertView.setTag(holder);
            } else {
                holder = (ViewHolder) convertView.getTag();
            }

            holder.photoImageView.setImageResource(R.mipmap.default_error);
            ...
            ImageLoader.getInstance().display(FileUtils.getPhotoImagePath(photo.getPath()), holder.photoImageView,
                    mWidth, mWidth);
        }
        return convertView;
    }
