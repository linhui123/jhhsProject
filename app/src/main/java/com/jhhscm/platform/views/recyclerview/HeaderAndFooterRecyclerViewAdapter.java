...
public class HeaderAndFooterRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onAttachedToRecyclerView(RecyclerView recyclerView) {
        ...
        if(manager instanceof ExStaggeredGridLayoutManager){
            final ExStaggeredGridLayoutManager exManager=((ExStaggeredGridLayoutManager)manager);
            exManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                @Override
                public int getSpanSize(int position) {
                    return (isHeader(position) || isFooter(position))
                            ? exManager.getSpanCount() : 1;
                }
            });
        }
    }
