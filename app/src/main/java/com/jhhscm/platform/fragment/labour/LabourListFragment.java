...
import com.jhhscm.platform.databinding.FragmentLabourListBinding;
 ...
 * 劳务招聘子页面
 */
public class LabourListFragment extends AbsFragment<FragmentLabourListBinding> {
    ...
    public static LabourListFragment instance(int type) {
        LabourListFragment view = new LabourListFragment();
    ...
    @Override
    protected FragmentLabourListBinding bindRootView(LayoutInflater inflater, ViewGroup container, boolean attachToRoot) {
        return FragmentLabourListBinding.inflate(inflater, container, attachToRoot);
    }
    ...
    public void onEvent(GetRegionEvent event) {
        if ((type == 0 && event.activity == 1)
                || (type == 1 && event.activity == 2)) {
...
//                                                province = getRegionBean.getResult().get(0).getId() + "";
    ...
    private class PrinviceAdapter extends AbsRecyclerViewAdapter<GetRegionBean.ResultBean> {
        ...
        public AbsRecyclerViewHolder<GetRegionBean.ResultBean> onCreateViewHolder(ViewGroup parent, int viewType) {
            if (type == 0) {
                return new GetRegionViewHolder(mInflater.inflate(R.layout.item_mechanics_privince, parent, false), 1);
            } else {
                return new GetRegionViewHolder(mInflater.inflate(R.layout.item_mechanics_privince, parent, false), 2);
            }
        }
    }
