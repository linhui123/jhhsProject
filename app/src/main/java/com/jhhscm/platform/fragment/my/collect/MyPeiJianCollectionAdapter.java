...
public class MyPeiJianCollectionAdapter extends RecyclerView.Adapter<MyPeiJianCollectionAdapter.RecViewholder> {

    private Context context;
    private List<FindCollectListBean.DataBean> data = new ArrayList<>();
    private LayoutInflater layoutInflater;
    private MyPeiJianCollectionAdapter.DeletedItemListener deletedItemListener;

    public void setDeletedItemListener(MyPeiJianCollectionAdapter.DeletedItemListener deletedItemListener) {
        this.deletedItemListener = deletedItemListener;
    }

    public MyPeiJianCollectionAdapter(Context context) {
    ...
    @Override
    public MyPeiJianCollectionAdapter.RecViewholder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = layoutInflater.inflate(R.layout.item_my_collect, parent, false);
        return new MyPeiJianCollectionAdapter.RecViewholder(view);
    }

    @Override
    public void onBindViewHolder(final MyPeiJianCollectionAdapter.RecViewholder holder, final int position) {
        ...
        holder.rl.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                H5PeiJianActivity.start(context, url, "配件详情", data.get(position).getName(), data.get(position).getGood_code(), data.get(position).getPic_url(), data.get(position).getCounter_price(), 3);
            }
        });
    }
